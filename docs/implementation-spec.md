...
The system reads incomplete Canvas planner items plus missing submissions, an active Notion Work database, and a bounded iCalendar window. It normalizes those sources, assigns stable identities, and deterministically classifies work into Must, Smart, May, and Verify views. A bounded language-model request may add one-sentence guidance but cannot select, omit, reorder, rename, or re-estimate tasks.
